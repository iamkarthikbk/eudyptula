...
MODULE_DESCRIPTION("[a24a6bdd6a14] [SYSFS] Task 09 of The Eudyptula Challenge");
...
static ssize_t id_write(struct kobj *kobj, struct kobj_attribute *attr, const char *buff, size_t count)
{
	char *print_str = TLF_ID;
...
static struct kobj_attribute jiffies_attribute = __ATTR(jiffies, 0444, jiffie_read, jiffies_write);
...
static int __init my_init(void)
{
	...
	my_kobj = kobject_create_and_add("eudyptula", kernel_kobj); //KOBJ
	if (!my_kobj)
		return -ENOMEM;
	...
	pr_debug("Hello World! I'm task 08 of The Eudyptula Challenge");
	return retval;
}
